  ...
  function($timeout, $ntdConfig) {
    ...
    var regexpMap = function regexpMap(language) {
      ...
      return {
        ...
        'M':
          '(?:' + $.fn.datepicker.dates[language].monthsShort.join('|') + ')',
        'yyyy':
    ...
    return {
      ...
      link: function postLink(scope, element, attrs, controller) {
        ...
        var language = options.language || 'en',
            readFormat = attrs.dateFormat ||
            options.format ||
            ($.fn.datepicker.dates[language] &&
            $.fn.datepicker.dates[language].format) ||
            'mm/dd/yyyy',
        ...
        if (controller) {
          ...
          controller.$formatters.unshift(function(modelValue) {
            return type === 'date' &&
              ...
              modelValue ?
              $.fn.datepicker.DPGlobal.parseDate(
                modelValue,
                $.fn.datepicker.DPGlobal.parseFormat(readFormat),
                language
              ) :
              modelValue;
          });
          ...
          controller.$parsers.unshift(function(viewValue) {
            if (!viewValue) {
              ...
              return viewValue;
                ...
                dateFormatRegexp.test(viewValue)) {
              controller.$setValidity('date', true);
            ...
            if (isAppleTouch) {
              var date = controller.$viewValue ?
                $.fn.datepicker.DPGlobal.formatDate(
                  ...
                  $.fn.datepicker.DPGlobal.parseFormat(format), language) : '';
              element.val(date);
              return date;
            }
            ...
            element.on('changeDate', function(ev) {
              scope.$apply(function() {
                controller.$setViewValue(
                  type === 'string' ? element.val() : ev.date);
              });
            });
          }
